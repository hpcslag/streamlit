...
def test_update_component_merge_and_name_enforcement():
    ...
    # Attempt to change name and update only js and css override
    reg.update_component(
        "comp",
        {"name": "different", "js": "new-js", "css_asset_relative_path": "x.css"},
    )

    d = reg.get("comp")
    assert d is not None
    # Name is enforced to the registry key
    assert d.name == "comp"
    # css preserved; js updated; css override set
    assert d.css_content == "orig-css"

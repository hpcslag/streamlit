...
class BidiComponentRegistry:
    ...
    def update_component(
        ...
        """Update a stored component definition with new data.

        Called by collaborators such as file watchers when component assets
        change. Missing keys are preserved from the existing definition, while
        keys present with ``None`` explicitly clear those fields.
        ...
        updated_definition_data : dict[str, Any]
            New definition data for the component, potentially including
            ``html``, ``css``, ``js``, ``css_asset_relative_path``, and
